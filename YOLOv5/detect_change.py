...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            '''if cls > 0 and connected == 0 and lidar.getDistance() <= 50:
                print('warning')
                speak(option, msg)
            else:
                print(lidar.getDistance())'''
            #####라이더 부분 지우면 카메라 되고 있으면 카메라 안되고 라이다만 된다.
            '''def speak(option, msg):
                os.system("espeak {} '{}'".format(option, msg))
                
            if connected == 0 :
               while(1):
                #if lidar.getDistance() <= 50:
                if cls > 0  and lidar.getDistance() <= 50 :
                    print('warning')
                    speak(option, msg)
                else :
                    print(lidar.getDistance())'''
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if (f'{data.decode()}' == 'red out'):
            print('start')  ########
            msg1 = 'green light'
            #def speak(option, msg):
            os.system("espeak {} '{}'".format(option, msg1))
                #speak(option, msg1)

            opt = parse_opt()
            main(opt)
            if (f'{data.decode()}' == 'green out'):
                print('end')  ##############
                exit()
        else:
            # print(exit)
